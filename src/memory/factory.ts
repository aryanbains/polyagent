...
export class UnsupportedMemoryBackendError extends Error {
	constructor(backend: string) {
		super(`${backend} memory is not implemented yet. Use chroma, skip, or POLYAGENT_MEMORY_DRIVER=local for local development.`);
		this.name = 'UnsupportedMemoryBackendError';
	}
}

export function createMemoryStore(config: PolyagentConfig): MemoryStore {
	if (process.env.POLYAGENT_MEMORY_DRIVER === 'local') {
...
export async function getMemoryStats(config: PolyagentConfig | null): Promise<MemoryStats> {
	if (config === null) {
		return {
			backend: 'none',
			totalEmbeddings: 0,
