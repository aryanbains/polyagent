...
function artifactSlug(task: string): string {
	...
	return meaningful.slice(0, 5).join('-') || 'polyagent-output';
}
...
function createHtmlReport(task: string, records: AgentStepRecord[], finalOutput: string): string {
	...
	return [
		...
		'<main>',
		'<header>',
		`<p class="meta">Generated by Polyagent</p>`,
...
function stepTimeoutMs(options: MultiAgentRunOptions): number {
	...
	const value = Number.parseInt(process.env.POLYAGENT_AGENT_STEP_TIMEOUT_MS ?? '', 10);
...
function maxStepRetries(options: MultiAgentRunOptions): number {
	...
	const value = Number.parseInt(process.env.POLYAGENT_AGENT_STEP_RETRIES ?? '', 10);
